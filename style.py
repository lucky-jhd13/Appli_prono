...
def couleur_prob(p: float) -> str:
    """Retourne une couleur hex selon le niveau de probabilité (centralisé pour le style)."""
    if p >= 0.55: return "#10b981" # succés
    if p >= 0.40: return "#f59e0b" # warning
    return "#ef4444"               # danger

def injecter_css(c: dict):
    """Injecte le style CSS moderne pour toute l'application."""
    st.markdown(f"""
<style>
...
/* REFRESH/SIDEBAR BUTTON SPECIAL STYLE */
section[data-testid="stSidebar"] .stButton button {{
    height: 3rem !important;
    width: 100% !important;
}}

/* INPUTS (Selectbox, slider, etc.) */
...
.glass-card:hover {{
    ...
    box-shadow: 0 12px 40px {c['shadow']};
}}
...
.stat-value {{ font-family: 'Outfit', sans-serif; font-size: 1.8rem; font-weight: 700; color: {c['txt']}; display: block; }}
...
.verdict-title {{ font-family: 'Outfit', sans-serif; font-size: 0.95rem; font-weight: 600; text-transform: uppercase; letter-spacing: 1.5px; color: {c['txt']}; opacity: 0.7; margin-bottom: 12px; }}
.verdict-main {{ font-family: 'Outfit', sans-serif; font-size: 2.2rem; font-weight: 800; color: {c['txt']}; }}
.verdict-prob {{ font-size: 0.95rem; opacity: 0.7; margin-top: 6px; font-weight: 500; }}
...
/* KELLY STATS BOX */
.kelly-box     {{ background: linear-gradient(135deg, {c['success']}11, {c['success']}22); border: 1px solid {c['success']}55; border-radius: 16px; padding: 16px; margin-top: 16px; font-size: 0.95rem; font-weight: 500; box-shadow: 0 2px 10px rgba(16,185,129,0.1); }}
.kelly-box-neg {{ background: {c['card']}; border: 1px dashed {c['brd']}; border-radius: 16px; padding: 16px; margin-top: 16px; opacity: 0.6; font-size: 0.9rem; font-weight: 400; }}

/* HEATMAP NOTATION */
.heatmap-note {{ font-size: 0.85rem; opacity: 0.6; text-align: center; margin-top: 10px; font-weight: 500; }}
